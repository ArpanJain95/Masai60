...
function mainNavbarCont(mainNavbarLinks, isLoginPage, isCartPage) {
  ...
  if (!isCartPage) {
    ...
    mainNavbarLinks.forEach((linkData) => {
      ...
      link.href = `${linkData.url}?category=${linkData.category}`;
    ...
    if(userTokenString) {
      cartBtn.href = `./cart.html?id=${user.id}`;
    } else {
      cartBtn.href = `./login.html`;
    }
    ...
    const mainNavbarContainer = document.createElement("div");
    mainNavbarContainer.id = "mainNavbar-container";
    mainNavbarContainer.innerHTML = `
      <div id="mainNavbar-left-cont">
        <div id="mainNavbar-logo">
          <a href="./">
            <img src="./image/ic-desktop-bwkf-trademark-logo-bday-y24.svg" alt="bewakoof logo"></img>
          </a>
        </div>
      </div>
    `;
